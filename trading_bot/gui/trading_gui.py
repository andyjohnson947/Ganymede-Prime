...
class TradingGUI:
    ...
    def _connect_mt5(self):
        ...
        if self.mt5_manager.connect():
            ...
            self._log("✅ Connected to MT5 successfully")
    ...
    def _reload_config(self):
        ...
        try:
            ...
            if success:
                self._log("✅ Configuration reloaded successfully!")

                # If strategy is running, call its reload method
                if self.strategy:
                    self.strategy.reload_config()
                    self._log("✅ Strategy configuration updated")
                else:
                ...
                self._log("❌ Failed to reload configuration")
                messagebox.showerror("Error", "Failed to reload configuration")

        except Exception as e:
            self._log(f"❌ Config reload error: {str(e)}")
